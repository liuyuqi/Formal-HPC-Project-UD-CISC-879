...
node_t testnode [NUM + 1]; /*测试节点*/

int dag [NUM][NUM]; /*DAG图的邻接矩阵*/
int dag_new [NUM+1][NUM+1]; /*转换后的邻接矩阵*/
int dag_dynamic [NUM+1][NUM+1]; /*用于任务可执行性检测的邻接矩阵*/
int e_power [NUM+1][NUM+1]; /*DAG图中边的权值矩阵*/
int e_power_new [NUM+1][NUM+1]; /*转换后的边权值矩阵*/
...
#define NCORE 8/*处理器核心数目*/
core_t core[NCORE];/*建立一个多核处理器*/
...
int main (void)
{
    ...
    node_t* p_node = testnode; /*节点指针*/
    /*初始化阶段*/
    node_init (p_node); /*节点的初始化*/
    dag_init (); /*DAG图邻接矩阵的初始化*/ 
    epower_init (); /*DAG图边权值矩阵的初始化*/
    core_init ();	/*处理器模型的初始化*/
    
    /*随机数生成阶段*/
    p_node = testnode;
    empower_node (p_node);/*随机生成节点权值*/
    dag_create ();/*DAG图邻接矩阵的随机生成*/ 
    empower_epower ();/*DAG图边权值的随机生成*/
    dag_makeup (); /*将DAG补全成AOE网络*/
    //dag_print ();
    /*调度算法实施阶段*/
    ...
    for (var = 0; var <= 0; var++) {
        ...
        node_recover (); /*节点的初始化*/
        list_init ();
        convert_dag (); /*DAG图权值的变换*/
        createadjlist ();
        critical_path ();  /*AOE关键路径的求解*/
